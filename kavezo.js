...
function div() {
  ...
  for (var j in kavek) {
    ...
    btn.id = kavek[j].fajta;
    btn.onclick = function () {
      switch (this.id) {
      case 'arabica': counterA++; break;
      case 'columbia': counterB++;  break;
      case 'india': counterC++;    break;
      case 'ecuador': counterD++;  break;
      }
      if (counterA > 0) alert(counterA + ' arabica kávé rendelve');
      if (counterB > 0) alert(counterB + ' columbia kávé rendelve');
      if (counterC > 0) alert(counterC + ' india kávé rendelve');
      if (counterD > 0) alert(counterD + ' ecuador kávé rendelve');
      return false;
    };
